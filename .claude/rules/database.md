...
users             : id, email, username, full_name, first_name(TEXT NULL), last_name(TEXT NULL),
                    ...
                    ⚠️ full_name = concaténation maintenue (first_name + last_name) → lectures feed/profil intactes
                       ...
                       · bio = ORA-085 ✅ appliquée · client isolé (lib/profileBio.ts)
...
claims            : id, user_id → users.id, type(weight|sessions), exercise_id → exercises.id (NULL si sessions),
                    ...
                    scope(next_session|week|month|custom — ORA-082), deadline(TIMESTAMPTZ NULL), status(active|succeeded|failed|expired),
...
- **[ORA-082] `claims.scope` étendu** `week|month|custom` — ✅ appliquée (21/06/2026). `month` = J+30, `custom` = échéance choisie par l'utilisateur (`deadline` arbitraire). Résolution pilotée par `deadline` (succès si cible atteinte avant échéance ; échec à l'échéance via cron ORA-077 + `expireOverdueClaims`). `next_session` inchangé (deadline NULL, résolu au save).
- **[ORA-083] `claim_likes` + `claim_comments`** — ✅ appliquée (21/06/2026). Un claim résolu (succeeded/failed) refait surface dans le feed comme une publication → mêmes interactions qu'une activité (likes + commentaires, helpers `lib/claims.ts`, agrégés dans `useFeedData`). Distinct de `claim_votes` (pronostics, réservés aux claims actifs). Le feed affiche aussi le moment d'annonce (`created_at`) sur ces claims, et le corps dit « Claim validé » (plus « a tenu son claim »). RLS gouvernée par la visibilité du claim parent ; pas de like par commentaire (table dédiée non créée).
