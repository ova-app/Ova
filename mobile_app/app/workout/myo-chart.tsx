...
function FamilyRadar({
  ...
  const avgPath = useMemo(() => makeAvgPath(avgVals, cx, cy, maxR), [avgVals, cx, cy, maxR])
    ...
    <View style={{ width: size }}>
      ...
      <View style={[StyleSheet.absoluteFill, { width: size, height: size }]} pointerEvents="none">
        {labelPos.map((pos, i) => {
          ...
          const deltaColor =
            delta > 0 ? '#2da866' : delta < 0 ? '#b04040' : 'rgba(255,255,255,0.28)'
          return (
            <View
              key={i}
              style={[
                styles.labelWrap,
                { left: pos.x - LABEL_W / 2, top: pos.y - 12, width: LABEL_W },
              ]}
            >
              <Text
...
export default function MyoChart({
  ...
  const avgPath = useMemo(() => makeAvgPath(avgScores, cx, cy, maxR), [avgScores, cx, cy, maxR])
  const gridRings = useMemo(
    () => [0.25, 0.5, 0.75, 1.0].map((t) => makeGridRing(cx, cy, t * maxR)),
    [cx, cy, maxR]
  )
  ...
  const famAnimStyle = useAnimatedStyle(() => ({
    ...
    transform: [{ scale: famScale.value }, { translateY: famTy.value }],
  }))
              ...
              {famScores.map((sc, fi) => {
                ...
                return (
                  <Path
                    ...
                    strokeWidth={1.8}
                    strokeCap="round"
                    color={rgba(SECTOR_COLORS_HEX[fi], 0.52 + sc * 0.4)}
                  />
                )
              })}
              <Circle cx={cx} cy={cy} r={5} color="rgba(10,10,15,0.95)" />
